...
def proces(env, cant, cap, unit, io, RAM):
    for i in range(cant):
        ...
        nuevoP = newProcess(env, ('%s' % i), unit, RAM, IO, MEMORY, INST)
        env.process(nuevoP)
...
# P R O C E S O S
unit = simpy.Resource(env, capacity=2) #cantidad de procesadores
env.process(proces(env, PROCESOS, INTERVAL, unit, IO, RAM))
